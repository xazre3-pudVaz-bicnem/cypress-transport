...
export const jobs: Job[] = [
   ...
   */
  {
    ...
    description:
      "軽バン（軽貨物自動車）を使用した配送業務です。葛飾区を中心としたエリアで、集荷拠点でお預かりした荷物を届け先までお届けします。報酬は配達1個あたり160円以上の出来高制で、日額15,000円の最低保証があります。出来高が最低保証を下回った日も15,000円をお支払いします。ロイヤリティやシステム利用料の差し引きはありません。普通自動車免許（AT限定可）があれば従事でき、軽貨物の経験は問いません。車両をお持ちでない方には、リースの手配が可能です。",
    deliveryArea: "東京都葛飾区を中心としたエリア",
    salary: {
      text: "1個160円以上（出来高制）／日額15,000円の最低保証あり・ロイヤリティなし",
      /*
       * baseSalary は「確実に受け取れる下限」である最低保証を出す。
       * 出来高は配達個数によって変わり上限が決まっていないため、
       * minValue のみを設定し maxValue は入れない（架空の上限を作らない）。
       * 1個あたりの単価は unitText で表現できないため description 側で伝えている。
       */
      schema: { currency: "JPY", unitText: "DAY", minValue: 15000 },
    },
